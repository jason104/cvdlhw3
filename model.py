...
class DA_model(nn.Module):
    def __init__(self, n_classes, load_source_model=False, training=True, not_adaption=False, model_path='./model.pt') -> None:
        super().__init__()
        ...
        if load_source_model:
            ...
            msg = self.model.load_state_dict(checkpoint)
            print(msg)

        self.create_hooks()
        self.da_model = DomainAdaptationModule()
        self.training = training
        self.not_adaption = not_adaption
    ...
    def gen_instance_level_domain_labels(self, domain_labels, device):
        # TODO
        return torch.cat([torch.zeros(len(self.box_features[0])), torch.ones(len(self.box_features[1]))]).long().to(device)
    ...
    def forward(self, x1, gt1=None, x2=None):
        ...
        if self.training:
            ...
            instance_level_domain_labels = self.gen_instance_level_domain_labels(domain_labels, x1[0].device)
            ...
            self.box_features = torch.cat([self.box_features[0], self.box_features[1]])
                
            da_losses = self.da_model(img_features=backbone_feature_list, 
                                        da_ins_feature=self.box_features,
